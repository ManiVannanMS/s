...
def calc_hr_and_spo2(ir_data, red_data):
    ...
    peak_interval_sum = 0
    sq_peak_interval_sum = 0
    if n_peaks >= 2:
	print("Intervals")
        for i in range(1, n_peaks):
            interval = (ir_valley_locs[i] - ir_valley_locs[i-1])
	    sq_interval = interval*interval
	    sq_peak_interval_sum += sq_interval
	    peak_interval_sum += interval
	hrv = math.sqrt(sq_peak_interval_sum)
    ...
    for i in range(exact_ir_valley_locs_count):
        if ir_valley_locs[i] > BUFFER_SIZE:
            ...
            spo2_valid = False
            return hrv, hr, hr_valid, spo2, spo2_valid
